...
from .mixin.delivery import DeliveryMixin
...
class AdItem(db.Model, BaseModelMixin, CommentMixin, DeliveryMixin):
    __tablename__ = 'searchAd_bra_item'
    # 基础
    ...
    @property
    def start_date(self):
        return min([s.date for s in self.schedules]) if self.schedules.count() else datetime.date(1900, 1, 1)
    ...
    @property
    def end_date(self):
        return max([s.date for s in self.schedules]) if self.schedules.count() else datetime.date(1900, 1, 1)
    ...
    def schedule_sums_by_dates(self, dates_list):
        schedule_sums = [0] * len(dates_list)
        for s in self.schedules:
            if s.date in dates_list:
                schedule_sums[dates_list.index(s.date)] += s.num
        return schedule_sums
    ...
    def get_ctr(self, date):
        if date:
            return (str(round(float(self.get_click_num(date)) / float(self.get_monitor_num(date)),
                    2) * 100) + '%') if self.get_monitor_num(date) else 0.0
        else:
    ...
    def get_finshed_proper(self, date):
        if date:
            ...
            return (str(round(float(self.get_monitor_num(date)) / float(num), 2) * 100) + '%') if num else 0.0
        else:
            return (str(round(float(self.get_monitor_num_all()) / float(self.schedule_sum),
                    2) * 100) + '%') if self.schedule_sum else 0.0
...
class AdSchedule(db.Model, BaseModelMixin):
    __tablename__ = 'searchAd_bra_schedule'
