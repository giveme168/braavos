...
from searchAd.models.invoice import (searchAdAgentInvoice, searchAdAgentInvoicePay, AGENT_INVOICE_STATUS_PASS,
                            INVOICE_TYPE_CN)
from searchAd.forms.invoice import AgentInvoiceForm
from libs.signals import agent_invoice_apply_signal
from libs.paginator import Paginator
from searchAd.controllers.saler.searchAd_order.agent_invoice import (new_invoice as _new_invoice,
                                                          update_invoice as _update_invoice, get_invoice_from)


searchAd_finance_client_order_agent_pay_bp = Blueprint(
    'searchAd_finance_client_order_agent_pay', __name__, template_folder='../../../../templates/finance/client_order')
ORDER_PAGE_NUM = 50


@searchAd_finance_client_order_agent_pay_bp.route('/', methods=['GET'])
def index():
    ...
    return tpl('/finance/searchAd_order/agent_pay/index.html', orders=orders, title=u'申请中的代理打款',
...
@searchAd_finance_client_order_agent_pay_bp.route('/pass', methods=['GET'])
def index_pass():
    if not g.user.is_finance():
        abort(404)
    orders = list([
    ...
    return tpl('/finance/searchAd_order/agent_pay/index.html', orders=orders, title=u'已打的款媒体信息')


@searchAd_finance_client_order_agent_pay_bp.route('/<order_id>/info', methods=['GET'])
def info(order_id):
    if not g.user.is_finance():
        abort(404)
    ...
    return tpl('/finance/searchAd_order/agent_pay/info.html',
...
@searchAd_finance_client_order_agent_pay_bp.route('/<invoice_id>/pay_info', methods=['GET'])
...
@searchAd_finance_client_order_agent_pay_bp.route('/<invoice_id>/pay_num', methods=['POST'])
...
@searchAd_finance_client_order_agent_pay_bp.route('/<invoice_id>/pass', methods=['POST'])
...
@searchAd_finance_client_order_agent_pay_bp.route('/<order_id>/order/new', methods=['POST'])
def new_invoice(order_id, redirect_endpoint='searchAd_finance_client_order_agent_pay.info'):
    return _new_invoice(order_id, redirect_endpoint)


@searchAd_finance_client_order_agent_pay_bp.route('/<invoice_id>/update', methods=['POST'])
def update_invoice(invoice_id, redirect_endpoint='searchAd_finance_client_order_agent_pay.info'):
