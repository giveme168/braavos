...
searchAd_finance_client_order_invoice_bp = Blueprint(
    'searchAd_finance_client_order_invoice', __name__, template_folder='../../../../templates/finance/client_order')


ORDER_PAGE_NUM = 50


@searchAd_finance_client_order_invoice_bp.route('/', methods=['GET'])
def index():
...
@searchAd_finance_client_order_invoice_bp.route('/pass', methods=['GET'])
def index_pass():
    if not g.user.is_finance():
        abort(404)
    ...
    if type == 'excel':
        orders = set([invoice.client_order for invoice in searchAdInvoice.get_invoices_status(
            INVOICE_STATUS_PASS)])
        ...
        return response
    return tpl('/finance/searchAd_order/invoice/index_pass.html', orders=orders, locations=select_locations,
...
@searchAd_finance_client_order_invoice_bp.route('/<order_id>/info', methods=['GET'])
def info(order_id):
    if not g.user.is_finance():
        abort(404)
...
@searchAd_finance_client_order_invoice_bp.route('/<order_id>/order/new', methods=['POST'])
...
@searchAd_finance_client_order_invoice_bp.route('/<invoice_id>/update', methods=['POST'])
...
@searchAd_finance_client_order_invoice_bp.route('/<invoice_id>/invoice_num', methods=['POST'])
...
@searchAd_finance_client_order_invoice_bp.route('/<invoice_id>/pass', methods=['POST'])
