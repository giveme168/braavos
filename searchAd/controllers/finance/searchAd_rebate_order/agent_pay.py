...
from searchAd.models.rebate_order_invoice import (searchAdRebateAgentInvoice, searchAdAgentInvoicePay, AGENT_INVOICE_STATUS_PASS,
                            INVOICE_TYPE_CN)
...
from searchAd.controllers.saler.searchAd_rebate_order.agent_invoice import (new_invoice as _new_invoice,
                                                          update_invoice as _update_invoice, get_invoice_from)


searchAd_finance_rebate_order_agent_pay_bp = Blueprint(
    'searchAd_finance_rebate_order_agent_pay', __name__, template_folder='../../../../templates/finance/rebate_order')
ORDER_PAGE_NUM = 50


@searchAd_finance_rebate_order_agent_pay_bp.route('/', methods=['GET'])
def index():
...
@searchAd_finance_rebate_order_agent_pay_bp.route('/pass', methods=['GET'])
def index_pass():
    if not g.user.is_finance():
        abort(404)
    orders = list([
        invoicepay.agent_invoice.rebate_order for invoicepay in
        searchAdAgentInvoicePay.get_agent_invoices_status(AGENT_INVOICE_STATUS_PASS)])
    return tpl('/finance/searchAd_rebate_order/agent_pay/index.html', orders=orders, title=u'已打的款媒体信息')


@searchAd_finance_rebate_order_agent_pay_bp.route('/<order_id>/info', methods=['GET'])
def info(order_id):
    if not g.user.is_finance():
        abort(404)
    ...
    new_invoice_form.rebate_order.choices = [(order.id, order.name)]
    new_invoice_form.agent.choices = [(k.id, k.name) for k in order.agents]
    new_invoice_form.add_time.data = datetime.date.today()
    return tpl('/finance/searchAd_rebate_order/agent_pay/info.html',
...
@searchAd_finance_rebate_order_agent_pay_bp.route('/<invoice_id>/pay_info', methods=['GET'])
def pay_info(invoice_id):
    ...
    return tpl('/finance/searchAd_rebate_order/agent_pay/pay_info.html',
               form=form, invoice=invoice, reminder_emails=reminder_emails,
               INVOICE_TYPE_CN=INVOICE_TYPE_CN)


@searchAd_finance_rebate_order_agent_pay_bp.route('/<invoice_id>/pay_num', methods=['POST'])
def invoice_pay_num(invoice_id):
    ...
    invoice.rebate_order.add_comment(g.user,
                                     u"更新了打款金额:\n\r%s" % invoice.pay_money,
                                     msg_channel=5)
    return redirect(url_for("searchAd_finance_rebate_order_agent_pay.info", order_id=invoice.rebate_order.id))


@searchAd_finance_rebate_order_agent_pay_bp.route('/<invoice_id>/pass', methods=['POST'])
def invoice_pass(invoice_id):
    ...
    if action != 10:
        ...
        for invoice_pay in invoices_pay:
            ...
            flash(u'代理订单款已打,名称:%s, 打款金额%s' % (
                invoice_pay.agent_invoice.rebate_order.name +
                '-' + invoice_pay.agent_invoice.agent.name,
                str(invoice_pay.money)), 'success')
            invoice_pay.agent_invoice.rebate_order.add_comment(
    ...
    return redirect(url_for("searchAd_finance_rebate_order_agent_pay.pay_info", invoice_id=invoice_id))


@searchAd_finance_rebate_order_agent_pay_bp.route('/<order_id>/order/new', methods=['POST'])
def new_invoice(order_id, redirect_endpoint='searchAd_finance_rebate_order_agent_pay.info'):
    return _new_invoice(order_id, redirect_endpoint)


@searchAd_finance_rebate_order_agent_pay_bp.route('/<invoice_id>/update', methods=['POST'])
def update_invoice(invoice_id, redirect_endpoint='searchAd_finance_rebate_order_agent_pay.info'):
