...
searchAd_finance_rebate_order_data_query_bp = Blueprint(
    'searchAd_finance_rebate_order_data_query', __name__, template_folder='../../templates/finance/searchAd_order/data_query')


@searchAd_finance_rebate_order_data_query_bp.route('/agent_invoice', methods=['GET'])
def agent_invoice():
    ...
    if request.values.get('action', '') == 'download':
        ...
        return response
    return tpl('/finance/searchAd_rebate_order/data_query/index.html',
               orders=orders,
               year=year, month=month, info=info,
               title=u"已开客户发票", t_type='agent_invoice')


@searchAd_finance_rebate_order_data_query_bp.route('/back_money', methods=['GET'])
def back_money():
    now_date = datetime.datetime.now()
    info = request.values.get('info', '').strip()
    year = request.values.get('year', now_date.strftime('%Y'))
...
@searchAd_finance_rebate_order_data_query_bp.route('/back_invoice', methods=['GET'])
def back_invoice():
    now_date = datetime.datetime.now()
    info = request.values.get('info', '').strip()
    year = request.values.get('year', now_date.strftime('%Y'))
...
@searchAd_finance_rebate_order_data_query_bp.route('/rebate_agent_invoice', methods=['GET'])
...
@searchAd_finance_rebate_order_data_query_bp.route('/pay_rebate_agent_invoice', methods=['GET'])
