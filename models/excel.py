...
EXCEL_DATA_TYPE_MERGE = 0
EXCEL_DATA_TYPE_STR = 1
EXCEL_DATA_TYPE_NUM = 2
EXCEL_DATA_TYPE_FORMULA = 3
...
class ExcelCellItem(object):
    ...
    def __init__(self, type, data=" ", style=StyleFactory().style, merge_row=0, merge_col=0):
        self.type = type
        self.data = data
        self.style = style
        ...
        for row in range(0, len(excel_table)):
            ...
            for col in range(0, len(excel_table[row])):
                ...
                if cell.type == EXCEL_DATA_TYPE_FORMULA:
                    self.sheet.write_merge(
                        row, row + cell.merge_row, col, col + cell.merge_col, Formula(cell.data), cell.style)
        return self.xls
