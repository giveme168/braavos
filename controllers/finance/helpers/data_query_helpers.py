...
def write_order_excel(orders, t_type):
    ...
    if t_type in ['personal_outsource', 'outsource']:
        ...
        keys = [u'业务类型', u'代理/直客', u'客户', u'Campaign', u'直客销售', u'渠道销售', u'区域', u'合同号',
        ...
        th = 1
        for k in range(len(orders)):
            if orders[k].client_order.b_type == 1:
                b_type_name = u'增量'
            else:
                b_type_name = u'自营'
            worksheet.write(th, 0, b_type_name, align_left)
            worksheet.write(
                th, 1, orders[k].client_order.agent.name, align_left)
            worksheet.write(
                th, 2, orders[k].client_order.client.name, align_left)
            worksheet.write(th, 3, orders[k].client_order.campaign, align_left)
            worksheet.write(
                th, 4, orders[k].client_order.direct_sales_names, align_left)
            worksheet.write(
                th, 5, orders[k].client_order.agent_sales_names, align_left)
            worksheet.write(
                th, 6, orders[k].client_order.locations_cn, align_left)
            worksheet.write(th, 7, orders[k].client_order.contract, align_left)
            if t_type == 'pay_medium_invoice':
                worksheet.write(th, 8, orders[k].medium_invoice.company, align_left)
            elif t_type == 'medium_invoice':
                worksheet.write(th, 8, orders[k].company, align_left)
            elif t_type == 'medium_rebate_invoice':
                worksheet.write(th, 8, orders[k].company, align_left)
            else:
                worksheet.write(
                    th, 8, ','.join([m.name for m in orders[k].client_order.medium_orders]), align_left)
            worksheet.write(
                th, 9, orders[k].client_order.start_date_cn, align_left)
            worksheet.write(
                th, 10, orders[k].client_order.end_date_cn, align_left)
            worksheet.write(
                th, 11, orders[k].client_order.reminde_date_cn, align_left)
            worksheet.write(th, 12, orders[k].client_order.money, align_left)
            if t_type == 'agent_invoice':
                worksheet.write(th, 13, orders[k].money, align_left)
                worksheet.write(th, 14, orders[k].create_time_cn, align_left)
            elif t_type == 'back_money':
                worksheet.write(th, 13, orders[k].money, align_left)
                worksheet.write(th, 14, str(
                    orders[k].client_order.back_money_percent) + '%', align_left)
                worksheet.write(th, 15, orders[k].back_time_cn, align_left)
                worksheet.write(th, 16, str(
                    orders[k].real_back_money_diff_time) + u'天', align_left)
            elif t_type == 'back_invoice':
                worksheet.write(th, 13, orders[k].money, align_left)
                worksheet.write(th, 14, str(
                ...
                worksheet.write(th, 13, orders[k].money, align_left)
                worksheet.write(th, 14, orders[k].add_time_cn, align_left)
            elif t_type == 'pay_rebate_agent_invoice':
                worksheet.write(th, 13, orders[k].money, align_left)
                worksheet.write(th, 14, orders[k].pay_time_cn, align_left)
            elif t_type == 'medium_invoice':
                worksheet.write(th, 13, orders[k].money, align_left)
                worksheet.write(th, 14, orders[k].add_time_cn, align_left)
            elif t_type == 'pay_medium_invoice':
        ...
        if t_type == 'back_money':
            ...
            worksheet.merge_range(th, 13, th, 14, sum(
                [k.money for k in orders]), align_left)
    workbook.close()
