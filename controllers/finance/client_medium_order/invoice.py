...
from models.client_medium_order import ClientMediumOrder, CONTRACT_STATUS_CN
from models.invoice import (ClientMediumInvoice, INVOICE_STATUS_CN,
                            INVOICE_TYPE_CN, INVOICE_STATUS_PASS,
                            INVOICE_STATUS_APPLYPASS)
from libs.email_signals import invoice_apply_signal
from libs.paginator import Paginator
from forms.invoice import ClientMediumInvoiceForm
from controllers.finance.helpers.invoice_helpers import write_excel
from controllers.tools import get_download_response


finance_client_medium_order_invoice_bp = Blueprint(
    'finance_client_medium_order_invoice', __name__, template_folder='../../templates/finance/client_medium_order')


ORDER_PAGE_NUM = 50


@finance_client_medium_order_invoice_bp.route('/', methods=['GET'])
def index():
...
@finance_client_medium_order_invoice_bp.route('/pass', methods=['GET'])
def index_pass():
    if not g.user.is_finance():
        abort(404)
...
@finance_client_medium_order_invoice_bp.route('/<order_id>/info', methods=['GET'])
def info(order_id):
    if not g.user.is_finance():
        abort(404)
...
@finance_client_medium_order_invoice_bp.route('/<order_id>/order/new', methods=['POST'])
...
@finance_client_medium_order_invoice_bp.route('/<invoice_id>/delete', methods=['GET'])
...
@finance_client_medium_order_invoice_bp.route('/<invoice_id>/update', methods=['POST'])
...
@finance_client_medium_order_invoice_bp.route('/<invoice_id>/invoice_num', methods=['POST'])
...
@finance_client_medium_order_invoice_bp.route('/<invoice_id>/pass', methods=['POST'])
...
def get_invoice_from(invoice):
    invoice_form = ClientMediumInvoiceForm()
    invoice_form.client_medium_order.choices = [
