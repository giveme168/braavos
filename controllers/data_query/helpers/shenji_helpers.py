...
def write_client_order_excel(orders, year, total_money_data, total_money_rebate_data,
    ...
    if shenji:
        keys = [u'主体', u'区域', u'代理/直客', u'客户合同号', u'客户', u'Campaign', u'客户合同金额', u'执行开始',
                u'执行结束', u'回款日期', u'回款总金额', u'欠款总金额', u'客户发票', u'客户返点发票', u'已打款客户返点',
                u'售卖类型', u'代理/直客']
        y, x = 0, 0
        for k in range(len(keys)):
            worksheet.merge_range(y, x, y + 1, x, keys[k], align_center)
            x += 1
        worksheet.merge_range(0, 17, 0, 29, u'客户执行金额', align_center)
        for k in range(12):
            worksheet.write(1, 17 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 29, u'总计', align_center)
        worksheet.merge_range(0, 30, 0, 42, u'客户返点', align_center)
        for k in range(12):
            worksheet.write(1, 30 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 42, u'总计', align_center)
        worksheet.merge_range(0, 43, 0, 55, u'外包成本', align_center)
        for k in range(12):
            worksheet.write(1, 43 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 55, u'总计', align_center)
        worksheet.merge_range(0, 56, 1, 56, u'媒体供应商', align_center)
        worksheet.merge_range(0, 57, 1, 57, u'投放媒体', align_center)
        worksheet.merge_range(0, 58, 1, 58, u'媒体合同号', align_center)
        worksheet.merge_range(0, 59, 1, 59, u'媒体售卖金额', align_center)
        worksheet.merge_range(0, 60, 1, 60, u'媒体金额', align_center)
        worksheet.merge_range(0, 61, 1, 61, u'媒体发票', align_center)
        worksheet.merge_range(0, 62, 1, 62, u'媒体付款总金额', align_center)
        worksheet.merge_range(0, 63, 1, 63, u'媒体返点发票', align_center)
        worksheet.merge_range(0, 64, 1, 64, u'媒体返点回款', align_center)
        worksheet.merge_range(0, 65, 0, 77, u'媒体执行金额', align_center)
        for k in range(12):
            worksheet.write(1, 65 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 77, u'总计', align_center)
        worksheet.merge_range(0, 78, 0, 90, u'媒体返点', align_center)
        for k in range(12):
            worksheet.write(1, 78 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 90, u'总计', align_center)
        worksheet.merge_range(0, 91, 0, 103, u'合同利润', align_center)
        for k in range(12):
            worksheet.write(1, 91 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 103, u'总计', align_center)
        # 设置宽度为30
        for k in range(103):
        ...
        for k in range(len(orders)):
            ...
            if len(medium_data) > 1:
                ...
                worksheet.merge_range(
                    th, 12, th + len(medium_data) - 1, 12, orders[k]['invoice'], money_align_left)
                worksheet.merge_range(
                    th, 13, th + len(medium_data) - 1, 13, orders[k]['agent_invoice'], money_align_left)
                worksheet.merge_range(
                    th, 14, th + len(medium_data) - 1, 14, orders[k]['agent_invoice_pay'], money_align_left)
                worksheet.merge_range(
                    th, 15, th + len(medium_data) - 1, 15, orders[k]['resource_type_cn'], align_left)
                worksheet.merge_range(
                    th, 16, th + len(medium_data) - 1, 16, orders[k]['sale_type'], align_left)
                money_data = orders[k]['money_data']
                for i in range(len(money_data)):
                    worksheet.merge_range(
                        th, 17 + i, th + len(medium_data) - 1, 17 + i, money_data[i], money_align_left)
                worksheet.merge_range(
                    th, 29, th + len(medium_data) - 1, 29, sum(money_data), money_align_left)
                money_rebate_data = orders[k]['money_rebate_data']
                for i in range(len(money_rebate_data)):
                    worksheet.merge_range(
                        th, 30 + i, th + len(medium_data) - 1, 30 + i, money_rebate_data[i], money_align_left)
                worksheet.merge_range(
                    th, 42, th + len(medium_data) - 1, 42, sum(money_rebate_data), money_align_left)
                outsource_data = orders[k]['outsource_data']
                for i in range(len(outsource_data)):
                    worksheet.merge_range(
                        th, 43 + i, th + len(medium_data) - 1, 43 + i, outsource_data[i], money_align_left)
                worksheet.merge_range(
                    th, 55, th + len(medium_data) - 1, 55, sum(outsource_data), money_align_left)
                profit_data = orders[k]['profit_data']
                for i in range(len(profit_data)):
                    worksheet.merge_range(
                        th, 91 + i, th + len(medium_data) - 1, 91 + i, profit_data[i], money_align_left)
                worksheet.merge_range(
                    th, 103, th + len(medium_data) - 1, 103, sum(profit_data), money_align_left)
                for m in medium_data:
                    worksheet.write(th, 56, m['medium_group_name'], align_left)
                    worksheet.write(th, 57, m['name'], align_left)
                    worksheet.write(th, 58, m['medium_contract'], align_left)
                    worksheet.write(th, 59, m['sale_money'], money_align_left)
                    worksheet.write(th, 60, m['medium_money2'], money_align_left)
                    worksheet.write(th, 61, m['medium_invoice'], money_align_left)
                    worksheet.write(th, 62, m['medium_invoice_pay'], money_align_left)
                    worksheet.write(th, 63, m['medium_rebate_invoice'], money_align_left)
                    worksheet.write(th, 64, m['medium_order_back_money'], money_align_left)
                    medium_money2_data = m['medium_money2_data']
                    for i in range(len(medium_money2_data)):
                        worksheet.write(
                            th, 65 + i, medium_money2_data[i], money_align_left)
                    worksheet.write(th, 77, sum(
                    ...
                    for i in range(len(medium_money2_rebate_data)):
                        worksheet.write(
                            th, 78 + i, medium_money2_rebate_data[i], money_align_left)
                    worksheet.write(th, 90, sum(
                ...
                worksheet.write(th, 12, orders[k]['invoice'], money_align_left)
                worksheet.write(th, 13, orders[k]['agent_invoice'], money_align_left)
                worksheet.write(th, 14, orders[k]['agent_invoice_pay'], money_align_left)
                worksheet.write(th, 15, orders[k]['resource_type_cn'], align_left)
                worksheet.write(th, 16, orders[k]['sale_type'], align_left)
                money_data = orders[k]['money_data']
                for i in range(len(money_data)):
                    worksheet.write(th, 17 + i, money_data[i], money_align_left)
                worksheet.write(th, 29, sum(money_data), money_align_left)
                money_rebate_data = orders[k]['money_rebate_data']
                for i in range(len(money_rebate_data)):
                    worksheet.write(
                        th, 30 + i, money_rebate_data[i], money_align_left)
                worksheet.write(th, 42, sum(money_rebate_data), money_align_left)
                outsource_data = orders[k]['outsource_data']
                for i in range(len(outsource_data)):
                    worksheet.write(
                        th, 43 + i, outsource_data[i], money_align_left)
                worksheet.write(th, 55, sum(outsource_data), money_align_left)
                profit_data = orders[k]['profit_data']
                for i in range(len(profit_data)):
                    worksheet.write(th, 91 + i, profit_data[i], money_align_left)
                worksheet.write(th, 103, sum(profit_data), money_align_left)
                if medium_data:
                    worksheet.write(th, 56, medium_data[0]['medium_group_name'], align_left)
                    worksheet.write(th, 57, medium_data[0]['name'], align_left)
                    worksheet.write(th, 58, medium_data[0][
        ...
        worksheet.write(th, 11, sum([k['money'] - k['back_moneys']
                                     for k in orders]), money_align_left)
        worksheet.write(th, 12, sum([k['invoice'] for k in orders]), money_align_left)
        worksheet.write(th, 13, sum([k['agent_invoice'] for k in orders]), money_align_left)
        worksheet.write(th, 14, sum([k['agent_invoice_pay'] for k in orders]), money_align_left)
        worksheet.merge_range(th, 15, th, 16, '', align_center)
        for k in range(len(total_money_data)):
            worksheet.write(th, 17 + k, total_money_data[k], money_align_left)
        worksheet.write(th, 29, sum(total_money_data), money_align_left)
        for k in range(len(total_money_rebate_data)):
            worksheet.write(
                th, 30 + k, total_money_rebate_data[k], money_align_left)
        worksheet.write(th, 42, sum(total_money_rebate_data), money_align_left)
        for k in range(len(total_outsource_data)):
            worksheet.write(
                th, 43 + k, total_outsource_data[k], money_align_left)
        worksheet.write(th, 55, sum(total_outsource_data), money_align_left)
        worksheet.merge_range(th, 56, th, 58, '', align_center)
        worksheet.write(th, 59, sum([k['medium_sale_money']
                                     for k in orders]), money_align_left)
        worksheet.write(th, 60, sum([k['medium_medium_money2']
                                     for k in orders]), money_align_left)
        worksheet.write(th, 61, sum([k['medium_invoice']
                                     for k in orders]), money_align_left)
        worksheet.write(th, 62, sum([k['medium_invoice_pay']
                                     for k in orders]), money_align_left)
        worksheet.write(th, 63, sum([k['medium_rebate_invoice']
                                     for k in orders]), money_align_left)
        worksheet.write(th, 64, sum([k['medium_order_back_money']
                                     for k in orders]), money_align_left)
        for k in range(len(total_medium_money2_data)):
            worksheet.write(
                th, 65 + k, total_medium_money2_data[k], money_align_left)
        worksheet.write(th, 77, sum(total_medium_money2_data), money_align_left)
        for k in range(len(total_medium_money2_rebate_data)):
            worksheet.write(
                th, 78 + k, total_medium_money2_rebate_data[k], money_align_left)
        worksheet.write(th, 90, sum(
            total_medium_money2_rebate_data), money_align_left)
        for k in range(len(total_profit_data)):
            worksheet.write(th, 91 + k, total_profit_data[k], money_align_left)
        worksheet.write(th, 103, sum(total_profit_data), money_align_left)
    else:
        keys = [u'主体', u'区域', u'代理/直客', u'客户合同号', u'客户', u'Campaign', u'客户合同金额', u'执行开始',
