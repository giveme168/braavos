...
def write_client_order_excel(orders, year, total_money_data, total_money_rebate_data,
    ...
    if shenji:
        ...
        worksheet.merge_range(0, 40, 0, 52, u'外包成本', align_center)
        for k in range(12):
            worksheet.write(1, 40 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 52, u'总计', align_center)
        worksheet.merge_range(0, 53, 1, 53, u'媒体供应商', align_center)
        worksheet.merge_range(0, 54, 1, 54, u'投放媒体', align_center)
        worksheet.merge_range(0, 55, 1, 55, u'媒体合同号', align_center)
        worksheet.merge_range(0, 56, 1, 56, u'媒体售卖金额', align_center)
        worksheet.merge_range(0, 57, 1, 57, u'媒体金额', align_center)
        worksheet.merge_range(0, 58, 0, 70, u'媒体执行金额', align_center)
        for k in range(12):
            worksheet.write(1, 58 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 70, u'总计', align_center)
        worksheet.merge_range(0, 71, 0, 83, u'媒体返点', align_center)
        for k in range(12):
            worksheet.write(1, 71 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 83, u'总计', align_center)
        worksheet.merge_range(0, 84, 0, 96, u'合同利润', align_center)
        for k in range(12):
            worksheet.write(1, 84 + k, str(k + 1) + u'月', align_center)
        worksheet.write(1, 96, u'总计', align_center)
        # 设置宽度为30
        for k in range(96):
        ...
        for k in range(len(orders)):
            ...
            if len(medium_data) > 1:
                ...
                for i in range(len(outsource_data)):
                    worksheet.merge_range(
                        th, 40 + i, th + len(medium_data) - 1, 40 + i, outsource_data[i], money_align_left)
                worksheet.merge_range(
                    th, 52, th + len(medium_data) - 1, 52, sum(outsource_data), money_align_left)
                profit_data = orders[k]['profit_data']
                for i in range(len(profit_data)):
                    worksheet.merge_range(
                        th, 84 + i, th + len(medium_data) - 1, 84 + i, profit_data[i], money_align_left)
                worksheet.merge_range(
                    th, 96, th + len(medium_data) - 1, 96, sum(profit_data), money_align_left)
                for m in medium_data:
                    worksheet.write(th, 53, m['medium_group_name'], align_left)
                    worksheet.write(th, 54, m['name'], align_left)
                    worksheet.write(th, 55, m['medium_contract'], align_left)
                    worksheet.write(th, 56, m['sale_money'], money_align_left)
                    worksheet.write(th, 57, m['medium_money2'], money_align_left)
                    medium_money2_data = m['medium_money2_data']
                    for i in range(len(medium_money2_data)):
                        worksheet.write(
                            th, 58 + i, medium_money2_data[i], money_align_left)
                    ...
                    for i in range(len(medium_money2_rebate_data)):
                        worksheet.write(
                            th, 71 + i, medium_money2_rebate_data[i], money_align_left)
                    worksheet.write(th, 83, sum(
                ...
                for i in range(len(outsource_data)):
                    worksheet.write(
                        th, 40 + i, outsource_data[i], money_align_left)
                worksheet.write(th, 52, sum(outsource_data), money_align_left)
                profit_data = orders[k]['profit_data']
                for i in range(len(profit_data)):
                    worksheet.write(th, 84 + i, profit_data[i], money_align_left)
                worksheet.write(th, 96, sum(profit_data), money_align_left)
                if medium_data:
                    worksheet.write(th, 53, medium_data[0]['medium_group_name'], align_left)
                    worksheet.write(th, 54, medium_data[0]['name'], align_left)
                    ...
                    worksheet.write(th, 57, medium_data[0][
        ...
        for k in range(len(total_outsource_data)):
            worksheet.write(
                th, 40 + k, total_outsource_data[k], money_align_left)
        worksheet.write(th, 52, sum(total_outsource_data), money_align_left)
        worksheet.merge_range(th, 53, th, 55, '', align_center)
        worksheet.write(th, 56, sum([k['medium_sale_money']
                                     for k in orders]), money_align_left)
        worksheet.write(th, 57, sum([k['medium_medium_money2']
                                     for k in orders]), money_align_left)
        for k in range(len(total_medium_money2_data)):
            worksheet.write(
                th, 58 + k, total_medium_money2_data[k], money_align_left)
        worksheet.write(th, 70, sum(total_medium_money2_data), money_align_left)
        for k in range(len(total_medium_money2_rebate_data)):
            worksheet.write(
                th, 71 + k, total_medium_money2_rebate_data[k], money_align_left)
        worksheet.write(th, 83, sum(
            total_medium_money2_rebate_data), money_align_left)
        for k in range(len(total_profit_data)):
            worksheet.write(th, 84 + k, total_profit_data[k], money_align_left)
        worksheet.write(th, 96, sum(total_profit_data), money_align_left)
    else:
        keys = [u'主体', u'区域', u'代理/直客', u'客户合同号', u'客户', u'Campaign', u'客户合同金额', u'执行开始',
                u'执行结束', u'回款日期', u'回款总金额', u'欠款总金额', u'售卖类型', u'代理/直客']
        y, x = 0, 0
