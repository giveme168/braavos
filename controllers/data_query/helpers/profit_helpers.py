    ...
    for k in range(len(orders)):
        ...
        if len(mediums) > 1:
            ...
            for i in range(len(mediums)):
                ...
                worksheet.write(th, 10, mediums[i].medium_money2, align_left)
            ...
            if orders[k].medium_orders:
                ...
                worksheet.write(
                    th, 10, orders[k].medium_orders[0].medium_money2, align_left)
                ...
                worksheet.write(
                    th, 12, orders[k].medium_orders[0].rebate_medium_by_month(year, month), align_left)
            worksheet.write(
                th, 13, orders[k].profit_money(year, month), align_left)
            worksheet.write(th, 14, orders[k].start_date_cn, align_left)
            worksheet.write(th, 15, orders[k].end_date_cn, align_left)
            th += 1
    workbook.close()
    response.data = output.getvalue()
    filename = ("%s-%s.xls" %
