...
def _get_medium_moneys(orders, pre_monthes, medium_id):
    ...
    for d in pre_monthes:
        if medium_id:
            pro_month_orders = [o for o in orders.filter_by(
                month_day=d['month']) if o.status == 1 and o.order.medium_id == medium_id]
        else:
            pro_month_orders = [o for o in orders.filter_by(
                month_day=d['month']) if o.status == 1 and o.order.medium_id not in [3, 4, 5, 6, 7, 8, 9, 14, 21]]
        for k in range(1, 4):
            location_orders = [
            ...
            if location_orders:
                ...
                money2 = sum([k.medium_money2 / len(k.locations)
                              for k in location_orders])
                medium_rebate = sum(
                    [k.medium_money2 / len(k.locations) * k.order.medium_rebate / 100 for k in location_orders])
                m_ex_money = money2 - medium_rebate
                a_rebate = sum([k.medium_money2 / len(k.locations) *
                                k.client_order.agent_rebate / 100 for k in location_orders])
