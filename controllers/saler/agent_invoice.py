...
saler_agent_invoice_bp = Blueprint(
    'saler_agent_invoice', __name__, template_folder='../../templates')


@saler_agent_invoice_bp.route('/<order_id>/order', methods=['GET'])
def index(order_id):
    ...
    return tpl('/saler/agent/index.html', order=order, reminder_emails=reminder_emails,
...
def invoice(invoice_id):
    ...
    reminder_emails = [(u.name, u.email) for u in User.all_active()]
    return tpl('/saler/agent/invoice.html', form=form, invoice=invoice, reminder_emails=reminder_emails,
               INVOICE_TYPE_CN=INVOICE_TYPE_CN, AGENT_INVOICE_STATUS_CN=AGENT_INVOICE_STATUS_CN)


@saler_agent_invoice_bp.route('/<invoice_id>/invoice/pay/new', methods=['POST'])
def new_invoice_pay(invoice_id):
    ...
    if mi.pay_invoice_money + money > mi.money:
        ...
        return redirect(url_for('saler_agent_invoice.invoice', invoice_id=invoice_id))
...
@saler_agent_invoice_bp.route('/<invoice_id>/invoice/pay/<invoice_pay_id>/update', methods=['POST'])
...
@saler_agent_invoice_bp.route('/<invoice_id>/apply', methods=['POST'])
...
@saler_agent_invoice_bp.route('/<agent_id>/tax_info', methods=['POST'])
