...
from models.invoice import (ClientMediumInvoice, INVOICE_STATUS_CN, INVOICE_TYPE_CN,
...
saler_client_medium_order_invoice_bp = Blueprint(
    'saler_client_medium_order_invoice', __name__, template_folder='../../templates/saler')


@saler_client_medium_order_invoice_bp.route('/<order_id>/order', methods=['GET'])
def index(order_id):
...
@saler_client_medium_order_invoice_bp.route('/<order_id>/order/new', methods=['POST'])
def new_invoice(order_id):
    ...
    if request.method == 'POST' and form.validate():
        if int(form.money.data) > (int(order.money) - int(order.invoice_apply_sum) - int(order.invoice_pass_sum)):
            ...
            return redirect(url_for("saler_client_medium_order_invoice.index", order_id=order_id))
...
@saler_client_medium_order_invoice_bp.route('/<invoice_id>/update', methods=['POST'])
...
@saler_client_medium_order_invoice_bp.route('/<invoice_id>/apply', methods=['POST'])
