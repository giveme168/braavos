...
saler_client_order_agent_invoice_bp = Blueprint(
    'saler_client_order_agent_invoice', __name__, template_folder='../../templates')


@saler_client_order_agent_invoice_bp.route('/<order_id>/order', methods=['GET'])
def index(order_id):
...
@saler_client_order_agent_invoice_bp.route('/<order_id>/order/new', methods=['POST'])
...
@saler_client_order_agent_invoice_bp.route('/<invoice_id>/invoice', methods=['POST', 'GET'])
...
@saler_client_order_agent_invoice_bp.route('/<invoice_id>/invoice/pay/new', methods=['POST'])
...
@saler_client_order_agent_invoice_bp.route('/<invoice_id>/invoice/pay/<invoice_pay_id>/update', methods=['POST'])
...
@saler_client_order_agent_invoice_bp.route('/<invoice_id>/update', methods=['POST'])
def update_invoice(invoice_id, redirect_endpoint='saler_client_order_agent_invoice.index'):
    invoice = AgentInvoice.get(invoice_id)
    if not invoice:
        abort(404)
    form = AgentInvoiceForm(request.form)
...
@saler_client_order_agent_invoice_bp.route('/<invoice_id>/apply_pay', methods=['POST'])
...
@saler_client_order_agent_invoice_bp.route('/<invoice_id>/apply', methods=['POST'])
def apply_invoice(invoice_id):
    invoice = AgentInvoice.get(invoice_id)
    if not invoice:
        abort(404)
    invoices_ids = request.values.getlist('invoices')
    invoices = AgentInvoice.gets(invoices_ids)
    if not invoices:
        abort(403)
    emails = request.values.getlist('email')
    msg = request.values.get('msg', '')
    ...
    return redirect(url_for("saler_client_order_agent_invoice.index", order_id=invoice.client_order.id))


@saler_client_order_agent_invoice_bp.route('/<agent_id>/tax_info', methods=['POST'])
